...
extern void write_binary(char *name, unsigned char *buf, int len);
int main(int argc, char* argv[])
{
    ...
    if (argc != 3) {
        fprintf(stderr, "Provde message and signature to verify\n");
        ret = -1;
    }
    else {
        fp=fopen(argv[1], "rb");
        ...
        fread(signature, sigLen, 1, fp);
        fclose(fp);

        write_binary("msgverify.bin", msg, msgLen);
    }
    ...
    write_binary("digestverify.bin", digest, WC_SHA256_DIGEST_SIZE);
    ...
    if (ret == 0) {
        decSigLen = wc_RsaSSL_VerifyInline(signature, sigLen,
                                           &decSig, &rsaKey);
        if ((int)decSigLen < 0)
            ret = (int)decSigLen;
    }
